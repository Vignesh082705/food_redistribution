...
admin.initializeApp({
  ...
  databaseURL: "https://food-redistribution-595ee-default-rtdb.firebaseio.com"
});
...
async function checkPendingPickupExpiry() {
  const snapshot = await db.ref("donations").once("value");
  if (!snapshot.exists()) return;
...
// 🟢 Manual Trigger API (at bottom of the file)
import express from "express";
const app = express();

// Route to manually trigger donation expiry check
app.get("/trigger-check", async (req, res) => {
  await checkExpiringDonations();
  res.send("✅ Donation expiry check triggered manually!");
});
app.get("/check-rejected", async (req, res) => {
      await checkRejectedRequests();
      res.status(200).send("Checked and updated rejected requests.");
});

// Server will run on localhost:5000
app.listen(5000, () => {
  console.log("🚀 Manual trigger server running at http://localhost:5000");
});